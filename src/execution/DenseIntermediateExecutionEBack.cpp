...
DenseIntermediateExecutionEBack::DenseIntermediateExecutionEBack(ExperimentType& experiment) :
	_experiment(experiment), _train_set(), _test_set() {

}

void DenseIntermediateExecutionEBack::process(size_t refresh_interval) {
	_load_data();
...
Tensor<Time> DenseIntermediateExecutionEBack::compute_time_at(size_t i) const {
	throw std::runtime_error("Unimplemented");
}

void DenseIntermediateExecutionEBack::_load_data() {
	for(Input* input : _experiment.train_data()) {
		size_t count = 0;
		while(input->has_next()) {
			auto entry = input->next();
			_train_set.emplace_back(entry.first,entry.second);
			count ++;
		}
	...
	for(Input* input : _experiment.test_data()) {
		...
		while(input->has_next()) {
			auto entry = input->next();
			_test_set.emplace_back(entry.first,entry.second);
			count ++;
		}
...
void DenseIntermediateExecutionEBack::_process_train_data(AbstractProcess& process, std::vector<std::pair<std::string, Tensor<float>>>& data, size_t refresh_interval) {
...
void DenseIntermediateExecutionEBack::_process_test_data(AbstractProcess& process, std::vector<std::pair<std::string, Tensor<float>>>& data) {
...
void DenseIntermediateExecutionEBack::_process_output(size_t index) {
