																																		   ...
																																		   _cursor(0), _cursor_count(0), _label_count(0), _shape({VIDEO_WIDTH, VIDEO_HEIGHT, VIDEO_DEPTH, CONV_DEPTH}), _max_read(max_read)
{
	...
	// create parapmeter config file that can be edited by the user
	...
	std::string _filename = _file_path + "/Param_config/" + _exp_name_conf;

	if (!std::filesystem::exists(_filename))
		_create_param_file_with_default_parameters(_filename);
...
std::pair<std::string, Tensor<InputType>> Video::next()
{
	...
	// Tensor<float>::draw_nonscaled_tensor("/home/melassal/Workspace/CSNN/csnn-simulator-build/Input_frames/test/", out.second);
	return out;
}
...
/**
 * @brief creates a defualt file that contains parameters.
 *
 */
void Video::_create_param_file_with_default_parameters(std::string fileName)
{
	...
	DynamicJsonDocument doc(JSON_ARRAY_SIZE(150));

	doc["input_path"] = "";
	doc["t_obj"] = 0.65;
	doc["_frame_width"] = 80;
	doc["_frame_height"] = 60;
	doc["_video_frames"] = 1;
	doc["_train_sample_per_video"] = 0;
	doc["_test_sample_per_video"] = 0;
	doc["_th_mv"] = 0;
	doc["_frame_gap"] = 4;
	doc["_grey"] = 1;
	doc["_draw"] = 0;
	doc["filter_size"] = 5;
	doc["filter_number"] = 32;
	doc["tmp_filter_size"] = 1;
	doc["tmp_pooling_size"] = 1;
	doc["temp_stride"] = 1;
	doc["sampling_size"] = 800;
	doc["th_lr"] = 0.09f;
	doc["w_lr"] = 0.009f;
