...
DefaultOnOffTempFilter::DefaultOnOffTempFilter() : UniquePassProcess(_register_1), _expName(""),
												   _filter_size(0), _tmp_filter_size(0), _center_dev(0), _surround_dev(0), _draw(0), _height(0), _width(0), _depth(0), _conv_depth(0), _spacial_filter(), _temporal_filter()
{
...
DefaultOnOffTempFilter::DefaultOnOffTempFilter(std::string expName, size_t filter_size, size_t tmp_filter_size, float center_dev, float surround_dev, float center_tau, float surround_tau, size_t draw) : DefaultOnOffTempFilter()
{
	...
	if (_draw == 1)
		std::filesystem::create_directories("Input_frames/" + _expName + "/1DDoG/");
...
void DefaultOnOffTempFilter::process_train(const std::string &label, Tensor<float> &sample)
{
	_process(label, sample);
}

void DefaultOnOffTempFilter::process_test(const std::string &label, Tensor<float> &sample)
{
	_process(label, sample);
}

void DefaultOnOffTempFilter::_process(const std::string &label, Tensor<InputType> &in) const
{
	...
	in = out;
	if (_draw == 1)
		Tensor<float>::draw_nonscaled_tensor(_file_path + "/Input_frames/" + _expName + "/1DDoG/1DDoG_" + label + "_", out);
}
