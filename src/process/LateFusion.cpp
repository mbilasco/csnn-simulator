...
LateFusion::LateFusion(std::string expName, size_t draw, size_t fused_frames_number, size_t method) : LateFusion()
{
	...
	if (_method == 0)
	{
		...
		for (int k = 0; k < _depth; k++)
		{
			...
			for (int _i = 0; _i < _frames.size(); _i++)
			{
				...
				// imwrite("/home/melassal/Workspace/CSNN/csnn-simulator-build/Input_frames/test/frame_lf_"+ label+"_" + std::to_string(_i) + ".png", _frame);
			...
			if (_draw == 1 && k == 0)
				Tensor<float>::draw_tensor(_file_path + "/Input_frames/" + _expName + "/LF/LF " + label + ".png", out_prime);
	...
	else
	{
		...
		for (int k = 0; k < _depth; k++)
		{
			...
			for (int _i = 0; _i < _frames.size(); _i++)
			{
				cv::Mat _frame = _frames[_i];
				// imwrite("/home/melassal/Workspace/CSNN/csnn-simulator-build/Input_frames/test/frame_lf_"+ label+"_" + std::to_string(_i) + ".png", _frame);
				for (int c = 0; c < _height; c++) // with each loop take a new line
					_frame.col(c).copyTo(totalframe.col(c * _frames.size() + _i));
			}
			// fused output for one single frame, will be added to out.
			Tensor<InputType> out_prime(Shape({_height, _width * _fused_frames_number, 1, 1}));
			...
			totalframe = cv::Mat::zeros(_height, _width * _fused_frames_number, CV_32F);
		}
		in = out;
	}
}