...
OrientationAmplitude::OrientationAmplitude() : UniquePassProcess(_register_1), _expName(""), _draw(0), _scaler(1), _width(0), _height(0), _depth(0), _conv_depth(0)
{
...
void OrientationAmplitude::_process(const std::string &label, Tensor<InputType> &in) const
{
	...
	// This function returns matrices.
	if (in.shape().dim(2) >= 3)
	...
	for (int _i = 0; _i < _frames.size() - 1; _i++)
	{
		...
		{
			cv::cvtColor(_frames[_i], img1, cv::COLOR_BGR2GRAY);
			cv::cvtColor(_frames[_i + 1], img2, cv::COLOR_BGR2GRAY);
		}
