...
void MotionGridV1::_process(const std::string &label, Tensor<InputType> &in) const
{
	...
	int maxImagecolumn = HORIZONTAL_FRAMES; // ((HORIZONTAL_FRAMES / 4) - 1);
	...
	cv::Mat dirframe = cv::Mat::zeros(_height, _width * 4, CV_32FC1);						// 4 is the number of directions
	cv::Mat totalhoriz = cv::Mat::zeros(_height, _width * 4 * HORIZONTAL_FRAMES, CV_32FC1); // 4 is the number of directions
	cv::Mat totalframe = cv::Mat::zeros(_height * VERTICAL_FRAMES, _width * 4 * HORIZONTAL_FRAMES, CV_32FC1);
	...
	for (int _i = 0; _i < _frames.size() - 1; _i++)
	{
		...
		cv::hconcat(upframe, downframe, dirframe);
		cv::hconcat(dirframe, leftframe, dirframe);
		cv::hconcat(dirframe, rightframe, dirframe);
		...
		else if (imagecolumn < maxImagecolumn)
		{
			cv::hconcat(totalhoriz, dirframe, totalhoriz);
			dirframe = cv::Mat::zeros(_height, _width * 4, CV_32FC1); // 4 is the number of directions
			imagecolumn = imagecolumn + 1;
		}
		else if (imagecolumn >= maxImagecolumn)
		{
		...
		{
			totalframe = totalhoriz;
			imagerow = imagerow + 1;
		}
		else if (imagerow < maxImagerow && imagecolumn == 0)
		{
			cv::vconcat(totalframe, totalhoriz, totalframe);
			imagerow = imagerow + 1;
			cv::Mat totalhoriz = cv::Mat::zeros(_height, _width * 4 * HORIZONTAL_FRAMES, CV_32FC1); // 4 is the number of directions
		}
		else if (imagerow >= maxImagerow)
		{
			imagerow = 0;
			break;
		}
	...
	if (_frames_width != 0 || _frames_height != 0)
	{
		// imwrite("/home/melassal/Workspace/CSNN/csnn-simulator-build/Input_frames/test/totalhoriz_" + label + ".png", totalframe);
		cv::resize(totalframe, reducedtotalframe, _frame_size2);
		totalframe = reducedtotalframe;
	}

	// if (totalframe.channels() >= 3)
	// 	Tensor<float>::matrix_to_colored_tensor(totalframe, out);
	// else
	// 	Tensor<float>::matrix_to_colored_tensor(totalframe, out);

	if (totalframe.channels() >= 3)
		cv::cvtColor(totalframe, totalframe, cv::COLOR_BGR2GRAY);
