...
struct Handler
{
	...
	static void run(Experiment<T> &experiment, const std::map<std::string, float> &variables)
	{
		...
		experiment.template push<process::WhiteningPatches>(9, 0.01, 1.0, 2, 1000000);
		...
		experiment.template add_train<dataset::STL>(input_path + "train_X.bin", input_path + "train_y.bin");
		experiment.template add_test<dataset::STL>(input_path + "test_X.bin", input_path + "test_y.bin");
...
{
	Benchmark<Handler, SparseIntermediateExecution> bench(argc, argv, "bench7", 3);
	bench.add_variable("t_obj", {0.5f, 0.55f, 0.6f, 0.65f, 0.7f, 0.75f, 0.8f, 0.85f, 0.9f, 0.91f, 0.92f, 0.93f, 0.94f, 0.95f, 0.96f, 0.97f, 0.98f, 0.99f, 1.0f});
