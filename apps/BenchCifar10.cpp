...
/*
		...
		experiment.template add_preprocessing<process::Whitening>(0.01, 1.0, 1000);
		experiment.template add_preprocessing<process::SampleScaling>();
		experiment.template input<LatencyCoding>();

		experiment.template add_train<dataset::Cifar>(std::vector<std::string>({
		...
		experiment.template add_test<dataset::Cifar>(std::vector<std::string>({
		...
		auto& conv1 = experiment.template push_layer<layer::Convolution>("conv1", 5, 5, 128);
...
int main(int argc, char** argv) {
	/*
	Benchmark<Handler, OptimizedLayerByLayer> bench(argc, argv, "bench-cifar10-whiten", 1);
