...
int main(int argc, char **argv)
{
	for (int _repeat = 1; _repeat < 4; _repeat++) // RUN MORE MG exp UCF, RUN EXP THAT I DON4T LIKE THE RESULTS OF
	{
		...
		std::string _dataset = "UCF-j28-onoff24-" + std::to_string(_filter_size) + "-L2-2D1D";
		...
		size_t _frame_size_width = 240, _frame_size_height = 180;
		...
		size_t _th_mv = 0, _frame_gap = 3, _grey = 1, _draw = 0;
		// filter sizes
		size_t filter_size = _filter_size, tmp_filter_size = _filter_size;
		size_t filter_number = 64, tmp_pooling_size = 1, temp_stride = 1;
		...
		auto &conv1 = experiment.push<layer::Convolution3D>(filter_size, filter_size, 1, filter_number, "", 1, 1, temp_stride);
		...
		auto &conv2 = experiment.push<layer::Convolution3D>(1, 1, tmp_filter_size, filter_number, "", 1, 1, temp_stride);
		...
		conv2.parameter<bool>("log_spiking_neuron").set(true);
		...
		conv2_out.add_postprocessing<process::TemporalPooling>(2);
...
// experiment.push<process::DefaultOnOffFilter>(7, 1.0, 0.4);
...
// experiment.push<process::AddSaltPepperNoise>(experiment.name(), 50, 0);