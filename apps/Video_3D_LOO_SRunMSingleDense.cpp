...
int main(int argc, char **argv)
{
	...
	for (int _rep = 0; _rep < _repeat; _rep++) // RUN MORE MG exp UCF, RUN EXP THAT I DON4T LIKE THE RESULTS OF
	{
		//for (std::string subject : subjects)
		{
			...
			std::string _dataset = "IXMAS_" + std::to_string(start_time) + "_3D_" + subject + "_" + std::to_string(_filter_size) + "_" + std::to_string(_th)+ "_" + std::to_string(_rep) + "_" + std::to_string(_epochs);
