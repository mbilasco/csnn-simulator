...
namespace dataset
{
	 ...
	 * @param draw a flag that allows drawing the feature maps in a Visualization folder.
	 */
	class LoadSavedFeatures : public Input
	{

	public:
		LoadSavedFeatures(const std::string &folder_path, const size_t &draw = 0, size_t max_read = std::numeric_limits<size_t>::max());
	...
	private:
		...
		std::string _vis_path;
		std::string _vis_path2;
